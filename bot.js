...
// Pastikan foldernya ada dulu sebelum sqlite coba buka file di dalamnya
try {
...
const db = new sqlite3.Database(dbPath, (err) => {
  if (err) {
    ...
    console.log('Cek: apakah DB_PATH di Variables Railway sama persis dengan Mount Path Volume?');
  } else {
...
db.serialize(() => {
  ...
  // Migration database lama: tambah saldo
  ...
  db.run(`CREATE INDEX IF NOT EXISTS idx_topups_user ON topups(user_id)`);
...
const getMainMenu = (userId) => {
  ...
  const buttons = [
    ...
    [Markup.button.callback('💰 Saldo / Top Up', 'user_balance')]
  ];
...
const getAdminMenu = () => {
  return Markup.inlineKeyboard([
    ...
    [Markup.button.callback('💰 Top Up Pending', 'admin_topup_pending'), Markup.button.callback('🎁 Buat Voucher', 'admin_add_voucher')],
    [Markup.button.callback('🗑️ Hapus Voucher', 'admin_del_voucher')],
...
// SALDO & TOP UP MANUAL VIA QRIS
const getBalance = (userId, cb) => {
  db.get(`SELECT balance FROM users WHERE user_id = ?`, [userId], (err, row) => {
    cb(row ? Number(row.balance || 0) : 0);
  });
};
...
const sendBalance = async (ctx) => {
  ...
  getBalance(userId, async (balance) => {
    await safeClearAndSend(
      ctx,
      `💰 *SALDO AKUN*\n\nSaldo Anda: *Rp${balance.toLocaleString('id-ID')}*\n\nPilih nominal top up di bawah ini:`,
      Markup.inlineKeyboard([
        [Markup.button.callback('💵 Rp10.000', 'topup_amt_10000'), Markup.button.callback('💵 Rp25.000', 'topup_amt_25000')],
...
bot.action('user_balance', sendBalance);
bot.command('saldo', sendBalance);
bot.command('topup', sendBalance);

const createTopup = async (ctx, amount) => {
  amount = Number(amount);
  if (!Number.isInteger(amount) || amount < 1000 || amount > 10000000) {
    return ctx.reply('⚠️ Nominal top up harus antara Rp1.000 dan Rp10.000.000.');
  }

  db.get(`SELECT qris FROM store WHERE id = 1`, async (err, store) => {
    if (!store || !store.qris) {
      return ctx.reply('⚠️ Admin belum mengatur foto QRIS toko.');
    }
    ...
    db.run(
      `INSERT INTO topups (user_id, username, amount, status, created_at) VALUES (?, ?, ?, 'PENDING', ?)`,
      [ctx.from.id, username, amount, now],
      async function (err) {
        if (err) return ctx.reply('❌ Gagal membuat permintaan top up. Coba lagi.');
        const topupId = this.lastID;
        userState[ctx.from.id] = { step: 'TOPUP_UPLOAD_PROOF', topupId };

        const text =
          `💰 *TOP UP SALDO #${topupId}*\n\n` +
          `💵 Saldo masuk: *Rp${amount.toLocaleString('id-ID')}*\n` +
          `💳 Transfer: *Rp${finalAmount.toLocaleString('id-ID')}*\n\n` +
          `⚠️ Transfer *NOMINAL PAS* termasuk kode unik.\n` +
          `📸 Setelah transfer, kirim screenshot bukti pembayaran ke chat ini.\n\n` +
          `Saldo akan bertambah setelah admin menyetujui bukti pembayaran.`;

        try { await ctx.deleteMessage(); } catch (e) {}
        await ctx.replyWithPhoto(store.qris, {
          caption: text,
          parse_mode: 'Markdown',
          ...Markup.inlineKeyboard([[Markup.button.callback('❌ Batal', 'user_balance')]])
        });
      }
    );
  });
...
bot.action(/^topup_amt_(\d+)$/, async (ctx) => {
  ctx.answerCbQuery();
  await createTopup(ctx, Number(ctx.match[1]));
});

bot.action('topup_custom', async (ctx) => {
  ctx.answerCbQuery();
  userState[ctx.from.id] = { step: 'TOPUP_CUSTOM_AMOUNT' };
  await safeClearAndSend(
    ctx,
    `✏️ *NOMINAL TOP UP*\n\nKetik nominal yang ingin diisi.\nMinimal Rp1.000, maksimal Rp10.000.000.`,
    Markup.inlineKeyboard([[Markup.button.callback('❌ Batal', 'user_balance')]])
  );
});
...
// ADMIN: DAFTAR TOP UP MENUNGGU VERIFIKASI
bot.action('admin_topup_pending', async (ctx) => {
  if (Number(ctx.from.id) !== getAdminId()) return;
  db.all(`SELECT * FROM topups WHERE status = 'PENDING_REVIEW' ORDER BY id DESC LIMIT 20`, (err, rows) => {
    if (!rows || rows.length === 0) return ctx.reply('✅ Tidak ada top up yang menunggu verifikasi.');
    const buttons = rows.map(t => [
      Markup.button.callback(`#${t.id} • Rp${Number(t.amount).toLocaleString('id-ID')} • ${t.username || t.user_id}`, `topup_review_${t.id}`)
    ]);
    ctx.reply('💰 *TOP UP PENDING*\\n\\nPilih transaksi:', { parse_mode: 'Markdown', ...Markup.inlineKeyboard(buttons) });
  });
});

bot.action(/^topup_review_(\d+)$/, async (ctx) => {
  if (Number(ctx.from.id) !== getAdminId()) return;
  const id = ctx.match[1];
  db.get(`SELECT * FROM topups WHERE id = ?`, [id], async (err, t) => {
    if (!t) return ctx.answerCbQuery('Top up tidak ditemukan.', { show_alert: true });
    if (t.status !== 'PENDING_REVIEW') return ctx.answerCbQuery(`Status: ${t.status}`, { show_alert: true });

    const text = `💰 *TOP UP #${t.id}*\\n\\n` +
      `👤 *User:* ${t.username || '-'}\\n` +
      `🆔 *ID:* \`${t.user_id}\`\\n` +
      `💵 *Saldo masuk:* Rp${Number(t.amount).toLocaleString('id-ID')}\\n` +
      `📅 *Waktu:* ${t.created_at}`;
    ...
    if (t.proof) {
      await ctx.replyWithPhoto(t.proof, {
        caption: text,
        parse_mode: 'Markdown',
        ...Markup.inlineKeyboard([[
          Markup.button.callback('✅ Approve', `approve_topup_${t.id}`),
          Markup.button.callback('❌ Reject', `reject_topup_${t.id}`)
        ]])
      });
    } else {
...
bot.action(/^approve_topup_(\d+)$/, async (ctx) => {
  ...
  db.get(`SELECT * FROM topups WHERE id = ?`, [id], (err, t) => {
    if (!t) return ctx.answerCbQuery('Top up tidak ditemukan.', { show_alert: true });
    if (t.status === 'APPROVED') return ctx.answerCbQuery('Sudah di-approve.', { show_alert: true });
    if (t.status !== 'PENDING_REVIEW') return ctx.answerCbQuery('Top up bukan pending review.', { show_alert: true });

    db.serialize(() => {
      db.run('BEGIN TRANSACTION');
      db.run(`UPDATE users SET balance = COALESCE(balance, 0) + ? WHERE user_id = ?`, [t.amount, t.user_id]);
      db.run(`UPDATE topups SET status = 'APPROVED' WHERE id = ?`, [id], function (e) {
        if (e) {
          db.run('ROLLBACK');
          return ctx.answerCbQuery('❌ Gagal approve top up.', { show_alert: true });
        }
        db.run('COMMIT', async () => {
          getBalance(t.user_id, (balance) => {
            bot.telegram.sendMessage(t.user_id,
              `🎉 *TOP UP BERHASIL!*\n\n` +
              `💰 Saldo masuk: *Rp${Number(t.amount).toLocaleString('id-ID')}*\\n` +
          ...
          ctx.answerCbQuery('✅ Top up disetujui & saldo ditambahkan.', { show_alert: true });
          try { ctx.editMessageCaption(`✅ TOP UP #${id} APPROVED`); } catch (e) {}
          try { ctx.editMessageText(`✅ TOP UP #${id} APPROVED`); } catch (e) {}
        });
      });
    });
  });
...
bot.action(/^reject_topup_(\d+)$/, async (ctx) => {
  ...
  db.get(`SELECT * FROM topups WHERE id = ?`, [id], (err, t) => {
    ...
    db.run(`UPDATE topups SET status = 'REJECTED' WHERE id = ?`, [id], () => {
      bot.telegram.sendMessage(t.user_id,
        `❌ *TOP UP DITOLAK*\\n\\nPermintaan top up #${id} sebesar Rp${Number(t.amount).toLocaleString('id-ID')} ditolak admin.\\nSilakan hubungi Customer Service jika ini kesalahan.`,
        { parse_mode: 'Markdown' }
      ).catch(() => {});
      ctx.answerCbQuery('❌ Top up ditolak.', { show_alert: true });
      try { ctx.editMessageCaption(`❌ TOP UP #${id} REJECTED`); } catch (e) {}
      try { ctx.editMessageText(`❌ TOP UP #${id} REJECTED`); } catch (e) {}
    });
  });
});
...
// LIVE CHAT RELAY - universal, aman dari Markdown error
// Buyer yang sedang tidak berada di alur input akan diteruskan ke admin.
// Admin cukup REPLY pesan relay untuk membalas buyer.
bot.use(async (ctx, next) => {
  const adminId = getAdminId();
  const userId = ctx.from && ctx.from.id;
  if (!adminId || !userId) return next();

  // ADMIN -> BUYER: hanya jika membalas pesan relay yang tercatat di DB
  if (Number(userId) === adminId && ctx.message && ctx.message.reply_to_message) {
    const repliedMsgId = ctx.message.reply_to_message.message_id;
    const relay = await new Promise((resolve) => {
      db.get(`SELECT * FROM chat_relay WHERE admin_msg_id = ?`, [repliedMsgId], (err, row) => resolve(row || null));
    });
    if (relay) {
      try {
        await bot.telegram.copyMessage(relay.buyer_id, adminId, ctx.message.message_id);
        await ctx.reply('✅ Balasan terkirim ke buyer.');
      } catch (e) {
        await ctx.reply('⚠️ Gagal kirim balasan. Kemungkinan buyer sudah block bot.');
      }
      return;
    }
  }

  // ADMIN messages biasa tetap diproses handler admin/state di bawah.
  if (Number(userId) === adminId) return next();

  // Kalau buyer sedang dalam alur transaksi/input, jangan ganggu handler fitur lama.
  if (userState[userId]) return next();

  // BUYER -> ADMIN: copyMessage mempertahankan teks/foto/video/dokumen/voice/sticker dll.
  try {
    const buyerName = ctx.from.username ? `@${ctx.from.username}` : (ctx.from.first_name || 'Buyer');
    const header = `💬 Chat dari Buyer\n👤 ${buyerName}\n🆔 ID: ${userId}\n\n↩️ Reply pesan ini untuk membalas ke buyer.`;
    const sent = await bot.telegram.sendMessage(adminId, header);
    db.run(`INSERT OR REPLACE INTO chat_relay (admin_msg_id, buyer_id, buyer_name) VALUES (?, ?, ?)`, [sent.message_id, userId, buyerName]);
    await bot.telegram.copyMessage(adminId, ctx.chat.id, ctx.message.message_id);
    return;
  } catch (e) {
    console.log('LIVE CHAT relay error:', e.description || e.message);
  }
  return next();
});
...
bot.on('photo', async (ctx) => {
  ...
  if (state && state.step === 'TOPUP_UPLOAD_PROOF') {
    const topupId = state.topupId;
    delete userState[userId];

    db.get(`SELECT * FROM topups WHERE id = ?`, [topupId], (err, topup) => {
      if (!topup) return ctx.reply('⚠️ Permintaan top up tidak ditemukan.');
      db.run(`UPDATE topups SET proof = ?, status = 'PENDING_REVIEW' WHERE id = ?`, [photoId, topupId]);
      ctx.reply('✅ Bukti top up diterima! Tunggu admin memverifikasi.');

      if (adminId) {
        const reviewText = `💰 *KONFIRMASI TOP UP #${topupId}*\n\n` +
          `👤 *Buyer:* ${topup.username || '-'} (ID: ${topup.user_id})\n` +
          `💵 *Saldo masuk:* Rp${Number(topup.amount).toLocaleString('id-ID')}\n` +
        ...
        bot.telegram.sendPhoto(adminId, photoId, {
          ...
          parse_mode: 'Markdown',
          ...Markup.inlineKeyboard([[
            Markup.button.callback('✅ Approve Top Up', `approve_topup_${topupId}`),
            Markup.button.callback('❌ Reject', `reject_topup_${topupId}`)
          ]])
        }).catch(() => {});
      }
    });
...
bot.on('text', async (ctx) => {
  ...
  if (state && state.step === 'TOPUP_CUSTOM_AMOUNT') {
    const amountText = ctx.message.text.replace(/[^\d]/g, '');
    const amount = Number(amountText);
    delete userState[ctx.from.id];
    if (!Number.isInteger(amount) || amount < 1000 || amount > 10000000) {
      return ctx.reply('⚠️ Nominal tidak valid. Minimal Rp1.000 dan maksimal Rp10.000.000.', Markup.inlineKeyboard([[Markup.button.callback('💰 Coba Lagi', 'user_balance')]]));
    }
    await createTopup(ctx, amount);
    return;
  }
