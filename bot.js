...
// === TABEL BARU UNTUK FITUR JUAL SCRIPT BOT ===
...
await ctx.reply(
 `🆔 *INFO CHAT INI*\n\n`  +
 `Chat ID: \`${chat.id}\`\n`+` Tipe:  ${chatType} \n\n `+ (chatType === 'group' || chatType === 'supergroup' ?` 👉 Copy ID di atas (termasuk tanda minus \`-\` kalau ada), lalu paste ke menu Admin →  Grup Log/Testi . `:` Ini chat pribadi. Kalau mau ID grup, jalankan /cekid di dalam grup targetnya.`),
{ parse_mode: 'Markdown' }
);
});
...
// === HELPER FUNGSI DELIVERY SCRIPT ===
const deliverScript = async (ctx, script) => {
    try {
        if (script.instruction_text) {
            await ctx.replyWithMarkdown(`📝 *INSTRUKSI PEMBELIAN SCRIPT*\n\n${script.instruction_text}`);
        }
        if (script.video_file_id) {
            await ctx.replyWithVideo(script.video_file_id, { caption: '🎥 Video Tutorial Script Bot' });
        }
        if (script.zip_file_id) {
            await ctx.replyWithDocument(script.zip_file_id, { caption: '📂 File Source Code (ZIP)' });
        }
        await ctx.reply('✅ Terima kasih! Script telah dikirim. Silakan cek file di atas.', Markup.inlineKeyboard([[Markup.button.callback('🔙 Menu Utama', 'main_menu')]]));
    } catch (e) {
...
const deliverScriptById = async (userId, script) => {
    try {
        if (script.instruction_text) {
            await bot.telegram.sendMessage(userId, `📝 *INSTRUKSI PEMBELIAN SCRIPT*\n\n${script.instruction_text}`, { parse_mode: 'Markdown' });
        }
        if (script.video_file_id) {
            await bot.telegram.sendVideo(userId, script.video_file_id, { caption: '🎥 Video Tutorial Script Bot' });
        }
        if (script.zip_file_id) {
            await bot.telegram.sendDocument(userId, script.zip_file_id, { caption: '📂 File Source Code (ZIP)' });
        }
    } catch (e) {
        console.error('Gagal deliver script:', e.message);
    }
};

// MENU UTAMA (TOMBOL VPN DIHAPUS, DITAMBAH TOMBOL SCRIPT BOT)
...
// === HANDLER FITUR JUAL SCRIPT BOT (USER) ===
    ...
    db.get(`SELECT * FROM bot_scripts WHERE id = ?`, [scriptId], async (err, script) => {
        ...
        try {
            ...
            db.run(`INSERT INTO script_orders (user_id, username, script_id, amount, status, created_at, casaku_transaction_id, qris_expires_at) VALUES (?, ?, ?, ?, 'PENDING', ?, ?, ?)`,
                [userId, username, scriptId, script.price, now, qris.transactionId, qris.expiresAt], async function (dbErr) {
                    if (dbErr) {
...
return ctx.reply('⚠️ Nominal tidak valid. Top Up minimal Rp500 dan maksimal Rp10.000.000, masukkan angka 500–10000000:');
}
delete userState[ctx.from.id];
processTopUp(ctx, amount);
return;
...
if (state && state.step === 'SCRIPT_SET_INFO') {
    const parts = ctx.message.text.split('|');
    if (parts.length !== 2) {
        return ctx.reply('⚠️ Format salah! Gunakan: Nama|Harga\nContoh: Bot Toko Premium|50000');
    }
    const name = parts[0].trim();
    const price = parseInt(parts[1].trim());
    if (isNaN(price)) {
        return ctx.reply('⚠️ Harga harus angka!');
    }
    db.get(`SELECT id FROM bot_scripts WHERE id = 1`, (err, row) => {
        if (!row) {
            db.run(`INSERT INTO bot_scripts (id, name, price, stock) VALUES (1, ?, ?, 0)`, [name, price]);
        } else {
            db.run(`UPDATE bot_scripts SET name = ?, price = ? WHERE id = 1`, [name, price]);
        }
        delete userState[adminId];
        ctx.reply('✅ Nama dan harga script berhasil diupdate!');
    });
    return;
}
if (state && state.step === 'SCRIPT_SET_TEXT') {
    const text = ctx.message.text.trim();
    db.run(`UPDATE bot_scripts SET instruction_text = ? WHERE id = 1`, [text]);
    delete userState[adminId];
    ctx.reply('✅ Teks instruksi script berhasil diupdate!');
    return;
}
if (state && state.step === 'SCRIPT_ADD_STOCK') {
    const stock = parseInt(ctx.message.text.trim());
    if (isNaN(stock) || stock <= 0) {
        return ctx.reply('⚠️ Masukkan angka stok yang valid!');
    }
    db.get(`SELECT id FROM bot_scripts WHERE id = 1`, (err, row) => {
        if (!row) {
            return ctx.reply('⚠️ Script belum di-setting. Set nama dan harga terlebih dahulu.');
        }
        db.run(`UPDATE bot_scripts SET stock = stock + ? WHERE id = 1`, [stock]);
        delete userState[adminId];
        ctx.reply(`✅ Stok script berhasil ditambah ${stock}!`);
    });
    return;
}
...
db.get(`SELECT id FROM script_orders WHERE status = 'PENDING' AND casaku_transaction_id = ? LIMIT 1`, [transactionId], async (err, scriptOrderByTrx) => {
    const findScriptOrder = (callback) => {
        ...
        db.get(`SELECT id FROM script_orders WHERE status = 'PENDING' AND amount = ? AND (casaku_transaction_id IS NULL OR casaku_transaction_id = '') ORDER BY created_at ASC LIMIT 1`, [amount], (e, legacyScriptOrder) => callback(legacyScriptOrder));
    };
    ...
    findScriptOrder(async (scriptOrder) => {
        if (scriptOrder) {
            ...
            if (result.ok) {
                matchedType = 'script_order';
                matchedId = scriptOrder.id;
                console.log(`✅ Webhook Casaku: script order #${scriptOrder.id} auto-approved (Rp${amount}).`);
            }
        ...
        // Kalau bukan script order, cek order biasa
